...
migration-initial: ## Create initial Django migration. Example: make migration-initial app=core
	@if [ $(app) ]; then
		@echo ${LIGHT_YELLOW}'Creating initial Django migration for app $(app)...'${NC}
		@$(POETRY) run python src/manage.py makemigrations --empty $(app)
	else
...
#	@echo ${LIGHT_YELLOW}'step 3: pyright'${NC}
#	@$(POETRY) run pyright --project ./pyproject.toml --level error
